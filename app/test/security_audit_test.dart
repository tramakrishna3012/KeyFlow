...
void main() {
  ...
  group('Security Hardening Pass Verification Tests (SRS S-1 through S-7)', () {
    test(
      'S-1 Verification: SecureKeyStorage retrieves key from OS-native store without hardcoding',
      () async {
    ...
    test(
      'S-3 Verification: Exclusion list defaults include standard password manager identifiers',
      () {
    ...
    test(
      'S-4 Verification: Cloud translation requires explicit per-use approval (no covert networking)',
      () {
    ...
    test(
      'S-5 Verification: Codebase Log Audit asserts 0 instances of captured text in logger calls',
      () {
          ...
          for (final entity in libDir.listSync(recursive: true)) {
            if (entity is File && entity.path.endsWith('.dart')) {
              ...
              for (var i = 0; i < lines.length; i++) {
                final line = lines[i];
                if (logPattern.hasMatch(line) &&
                    sensitivePattern.hasMatch(line)) {
                  ...
                  if (!line.contains('length') &&
                    ...
                    // ignore: avoid_print
                    print(
                      'Potential S-5 Violation in ${entity.path}:${i + 1}: $line',
                    );
                  }
                }
              }
          ...
          if (serverFile.existsSync()) {
            ...
            for (var i = 0; i < lines.length; i++) {
              ...
              if (line.contains('console.log') &&
                ...
                // ignore: avoid_print
                print(
                  'Potential S-5 Violation in relay/server.js:${i + 1}: $line',
                );
              }
            }
          }
        ...
        expect(
          violations,
          equals(0),
          reason: 'S-5 Violation: Found logger call outputting captured text',
        );
      },
    );
  });
