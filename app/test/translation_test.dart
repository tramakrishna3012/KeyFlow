...
void main() {
  ...
  group('Translation Engine & Cloud Relay Tests (SRS FR-15..18)', () {
    test(
      'On-device translation returns local result with "Translated on-device" badge',
      () async {
    ...
    test(
      'Approved cloud translation returns "Translated via cloud (user approved)" badge',
      () async {
        ...
        expect(
          ...
          equals('Translated via cloud (user approved)'),
        );
      },
    );

    test(
      'Unreachable relay / offline network fails gracefully with clear error message',
      () async {
